...
async def discover_breagle_cover(
    scanner: type[BleakScanner] | None = None,
) -> list[dict[str, Any]]:
    """Scanning feature
    Scan the BLE neighborhood for an Breagle lift
...
class Breagle_btConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):  # type: ignore
    """Handle a config flow for breagle_bt."""
    ...
    async def async_step_scan(
        ...
        try:
            ...
            scanner = create_bleak_scanner(None, BluetoothScanningMode.ACTIVE, None)
            _LOGGER.debug("Using bleak scanner through HA")
        try:
            _LOGGER.debug("Starting a scan for Breagle Bt devices")
            ble_devices = await discover_breagle_cover(scanner)
    ...
    async def async_step_device(
        ...
        _LOGGER.debug(f"Breagle UniqueID: {unique_id}")
